    ...
    suburbs.each do |suburb|
      ...
      if suburb.state == state
        if (suburb.median_price_unit < max_price && suburb.median_price_unit > min_price) || (suburb.median_price_house < max_price && suburb.median_price_house > min_price)
          suburb_array.push({suburbName: suburb.suburb_name, housePrice: suburb.median_price_house, unitPrice: suburb.median_price_unit, lat: suburb.lat, long: suburb.long , state: suburb.state , suburb_coords: suburb.suburb_coords})
        end
      end
